...
from .backbones.densenet import densenet121, densenet161, densenet201
...
class Baseline(nn.Module):
    ...
    def __init__(self, num_classes, last_stride, model_path, neck, neck_feat, model_name, pretrain_choice, use_m=False):
        ...
        if self.use_m:
            if self.neck == 'no':
                self.m_classifier = nn.Linear(self.in_planes, 4)
            elif self.neck == 'bnneck':
                self.m_bottleneck = nn.BatchNorm1d(self.in_planes)
                self.m_bottleneck.bias.requires_grad_(False)  # no shift
                self.m_fc1 = nn.Linear(self.in_planes, 1024, bias=True)
                self.m_classifier = nn.Linear(1024, 4, bias=True)

                self.m_bottleneck.apply(weights_init_kaiming)
                self.m_fc1.apply(weights_init_kaiming)
                self.m_classifier.apply(weights_init_classifier)
    ...
    def forward(self, x):
        ...
        global_feat = self.gap(base_feat)  # (b, 2048, 1, 1)
        ...
        if self.training:
            ...
            m_fc = self.m_fc1(feat)
            m_score = self.m_classifier(m_fc)
            return cls_score, global_feat, m_score  # global feature for triplet loss
        else:
            if self.use_m:
